...
|Intel GPU Device Plugin|System Plugin|[Intel GPU device plugin](https://intel.github.io/intel-device-plugins-for-kubernetes/cmd/gpu_plugin/README.html) facilitates Kubernetes workload offloading by providing access to discrete (including Intel® Data Center GPU Flex & Max Series) and integrated Intel GPU devices supported by the host kernel.||
...
|OWASP Modsecurity CRS|Security|[OWASP Modsecurity CRS](https://github.com/coreruleset/modsecurity-crs-docker) is a set of generic attack detection rules for use with ModSecurity or compatible web application firewalls. It aims to protect web applications from a wide range of attacks, including the OWASP Top Ten, with a minimum of false alerts. Used in conjunction with [this traefik plugin](https://plugins.traefik.io/plugins/644d9a72ebafd55c9c740848/mx-m-owasp-crs-modsecurity-plugin) and [this dummy service](https://github.com/traefik/whoami) to protect every service in the lab.||
